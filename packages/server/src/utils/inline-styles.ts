...
export function getInlineStyles(cssFileName: string): string {
  try {
    ...
    const uiPackagePath = path.join(process.cwd(), "..", "..", "packages", "ui", "src")
    ...
    if (fs.existsSync(directPath)) {
      ...
      console.log(`[getInlineStyles] ✅ Found CSS at: ${directPath} (${cssContent.length} chars)`)
      return cssContent
    }
    ...
    for (const fallbackPath of fallbackPaths) {
      ...
      if (fs.existsSync(fallbackPath)) {
        ...
        console.log(`[getInlineStyles] ✅ Found CSS at fallback: ${fallbackPath} (${cssContent.length} chars)`)
        return cssContent
      }
    }
