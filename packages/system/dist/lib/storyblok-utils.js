 ...
 */
export function normalizeStoryblokPath(slug) {
 ...
 */
export function shouldIncludeStory(fullSlug) {
    // Exclude config/global stories
    if (fullSlug.includes("/config") ||
 ...
 */
export function getStoryPath(fullSlug) {
    if (!fullSlug)
        return "/";
    // Remove 'blog/' prefix
 ...
 */
export function determinePageType(slug) {
    if (!slug || slug.length === 0) {
        return "home";
    }
 ...
 */
export function extractTagSlug(slug) {
    if (!slug || slug.length !== 2 || slug[0] !== "posts") {
        return null;
    }
    return slug[1];
}
//# sourceMappingURL=storyblok-utils.js.map